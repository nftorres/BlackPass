"""
Este módulo contiene las funciones para mejorar la estética del 
programa y también para generar, hashear y guardar las contraseñas de 
BlackPass.
"""
import sys
import subprocess
...
def escribir(cadena, salto=True):
    """
    La función "escribir" imprime una cadena determinada carácter por
    carácter con un pequeño retraso entre cada carácter y un salto de 
    línea opcional al final.

    :param cadena: La cadena de entrada que debe imprimirse
    :param salto: Un parámetro booleano que determina si agregar o no 
    un salto de línea después de imprimir la cadena. Si se establece 
    en Verdadero, se agregará un salto de línea. Si se establece en 
...
def config_contraseña(longitud_contraseña):
    """
    Esta función solicita al usuario que elija un tipo de contraseña 
    (solo dígitos, solo letras o alfanumérica) y genera una contraseña 
    de la longitud especificada según la elección del usuario.

    :param longitud_contraseña: La longitud de la contraseña que el usuario quiere generar
    :return: una contraseña generada basada en la entrada del usuario 
    para el tipo de contraseña deseado (solo dígitos, solo letras o alfanumérico) 
    y la longitud deseada de la contraseña.
    """
    while True:
        print()
        config = input("¿Cómo quieres que sea tu contraseña"
                       "(Sólo digitos/Sólo letras/Alfanumérica)?: ")
        if (config.lower() == 'sólo dígitos' or
            config.lower() == 'solo digitos' or
            config.lower() == 'sólo digitos' or
                config.lower() == 'solo dígitos'):
            return generar_contraseña(longitud=longitud_contraseña,
                                       letras=False)
        elif (config.lower() == 'sólo letras' or
              config.lower() == 'solo letras'):
            return generar_contraseña(longitud=longitud_contraseña,
                                       digitos=False)
        elif (config.lower() == 'alfanumerica' or
              config.lower() == 'alfanumérica'):
            return generar_contraseña(longitud=longitud_contraseña)
        else:
...
def generar_contraseña(longitud=12, digitos=True, letras=True):
    """
    Esta función genera una contraseña aleatoria de una 
    longitud específica con una combinación de letras 
    y/o dígitos.

    :param longitud: La longitud de la contraseña que se va a generar. De forma 
    predeterminada, se establece en 12 caracteres, por defecto es 12(optional)
